...
# === Инициализация приложения и базы данных ДО использования db ===
app = Flask(__name__)
app.config.from_object(Config)

# Инициализация базы данных (должна быть до декораторов!)
db = Database()
...
# === Глобальный диспетчер (создается один раз) ===
bot = Bot(token=app.config['TELEGRAM_TOKEN'])
